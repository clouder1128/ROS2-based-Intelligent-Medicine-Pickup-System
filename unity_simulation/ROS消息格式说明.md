...
### 1. `/unity_commands` — 接收任务指令

**消息类型**: `task_msgs/TaskMsg`
...
- **脚本**: 回调处理见 `Assets/scripts/MyMsgScripts/TaskSubscriber.cs`
- **处理逻辑**: 收到任务后存入 `TaskList` 和 `TaskListData`，标记 `SceneManager.IsRefreshTask = true`
- **Topic 名称**: 注意带前导斜杠 `/unity_commands`
...
| `/unity_commands` | ← | `task_msgs/Task` | — | 接收任务指令（ROS 发布） |
