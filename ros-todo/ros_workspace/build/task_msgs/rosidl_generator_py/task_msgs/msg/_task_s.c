...
bool task_msgs__msg__task__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // cabinets
    PyObject * field = PyObject_GetAttrString(_pymsg, "cabinets");
    if (!field) {
      return false;
    }
    PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'cabinets'");
    if (!seq_field) {
      Py_DECREF(field);
      return false;
    }
    ...
    if (!task_msgs__msg__CabinetOrder__Sequence__init(&(ros_message->cabinets), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create task_msgs__msg__CabinetOrder__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    task_msgs__msg__CabinetOrder * dest = ros_message->cabinets.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!task_msgs__msg__cabinet_order__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * task_msgs__msg__task__convert_to_py(void * raw_ros_message)
{
  ...
  {  // cabinets
    ...
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->cabinets.data[i]);
      PyObject * pyitem = task_msgs__msg__cabinet_order__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
    ...
    {
      int rc = PyObject_SetAttrString(_pymessage, "cabinets", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
