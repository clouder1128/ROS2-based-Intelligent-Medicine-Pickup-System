# 医生审批系统 - doctor_cli.py 使用说明

## 概述

本文档介绍重构后的 `doctor_cli.py` 脚本功能和使用方法。该脚本现已升级为完整的交互式命令行界面，支持多次操作、添加医生建议、批准/拒绝审批单等功能。
...
### 启动交互模式
```bash
python doctor_cli.py
```

### 直接批准模式（向后兼容）
```bash
# 简单批准
python doctor_cli.py AP-20260408-XXXXXXX

# 带医生建议的批准
python doctor_cli.py AP-20260408-XXXXXXX --notes "饭后服用，多喝水" --doctor-id "doctor_001"
```

### 帮助信息
```bash
python doctor_cli.py -h
# 或
python doctor_cli.py --help
```
