...
def validate_and_get_drug(drug_id: int, num: int) -> tuple[dict | None, str | None]:
    ...
    if not drug:
        return None, f"Drug not found: id={drug_id}"
    if drug["expiry_date"] is not None and drug["expiry_date"] <= 0:
        return None, f'Drug expired: {drug["name"]}'
    if drug["quantity"] < num:
        return None, f'Insufficient stock: {drug["name"]} stock {drug["quantity"]}, need {num}'
    return drug, None
...
def find_drug_id_by_name(drug_name: str) -> int | None:
    ...
    try:
        conn = get_db_connection()
        cur = conn.execute("SELECT drug_id FROM inventory WHERE name = ?", (normalized,))
        row = cur.fetchone()
        if row:
            return row["drug_id"]
