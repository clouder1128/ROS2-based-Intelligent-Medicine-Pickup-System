...
def run_expiry_sweep() -> dict:
    """
    Advance "remaining days" by local date and clean up expired inventory (executed only once per day).
    Baseline date written by init_db to app_meta.expiry_sweep_date; old databases without this record
    will only write today's date without deduction on first access.
    Multiple days of shutdown will apply deduction for all missed days at once to avoid time loss.
...
def _boot_expiry_worker_once():
    """Start a single scheduled sweep thread in long-running service processes."""
...
if __name__ == "__main__":
    ...
    # debug + auto-reload: started by child process on import;
    # if this is the parent process (monitoring only, no reload), don't start here
