    ...
    try:
        ...
        for drug_id, num, drug in tasks:
            ...
            task_id, err = create_order_for_drug(
                conn, drug_id, num, drug, publish_now=False
            )
            if task_id is None:
                conn.rollback()
                return (
...
def dispense():
    ...
    try:
        ...
        for drug_id, quantity, drug_name in order_items:
            ...
            # Use helper function with publish_now=False (we'll publish after commit)
            task_id, err = create_order_for_drug(
                conn, drug_id, quantity, drug, publish_now=False
            )
            if task_id is None:
                conn.rollback()
                return (
