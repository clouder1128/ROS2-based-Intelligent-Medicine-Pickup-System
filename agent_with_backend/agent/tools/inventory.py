...
from database.pharmacy_client import get_all_drugs
...
def record_transaction(
    ...
    try:
        if transaction_type == "out":
            ...
            if order_result and order_result.get("success", False):
                response = {
                    ...
                    "drug_id": drug_id, "quantity": quantity,
                    "type": transaction_type, "reason": reason or "医生处方",
            ...
            response = {
                ...
                "transaction_id": f"TX-{datetime.now().strftime('%Y%m%d%H%M%S')}",
                "drug_id": drug_id, "quantity": quantity,
                "type": transaction_type, "reason": reason or "管理员补货",
                "timestamp": datetime.now().isoformat(),
        ...
        response = {
            ...
            "transaction_id": f"TX-{datetime.now().strftime('%Y%m%d%H%M%S')}",
            "drug_id": drug_id, "quantity": quantity,
            "type": transaction_type, "reason": reason or "未指定",
            "timestamp": datetime.now().isoformat(),
            "error": str(e), "note": "记录失败，backend连接异常",
        }
        return json.dumps(response, ensure_ascii=False, indent=2)
...
def get_stock_report(start_date: str = None, end_date: str = None, limit: int = 100) -> str:
    ...
    try:
        ...
        mock_report = [
            {"drug_name": "阿莫西林", "drug_id": 1, "start_stock": 120, "in": 50, "out": 70, "end_stock": 100, "transactions": 5},
            {"drug_name": "头孢克肟", "drug_id": 2, "start_stock": 90, "in": 30, "out": 40, "end_stock": 80, "transactions": 3},
        ]
...
def generate_purchase_suggestions() -> str:
    ...
    mock_suggestions = [
        {"drug_name": "阿莫西林", "drug_id": 1, "current_stock": 20, "min_stock_threshold": 50, "daily_avg_out": 5.2, "days_until_stockout": 3.8, "suggested_purchase_quantity": 100, "urgency": "高", "reason": "库存低于阈值且消耗速度较快"},
        {"drug_name": "头孢克肟", "drug_id": 2, "current_stock": 45, "min_stock_threshold": 50, "daily_avg_out": 2.1, "days_until_stockout": 21.4, "suggested_purchase_quantity": 60, "urgency": "中", "reason": "库存接近阈值"},
        {"drug_name": "蒙脱石散", "drug_id": 5, "current_stock": 30, "min_stock_threshold": 40, "daily_avg_out": 1.5, "days_until_stockout": 20.0, "suggested_purchase_quantity": 50, "urgency": "中", "reason": "库存低于阈值"},
    ]
    response = {
        "generated_at": datetime.now().isoformat(),
        ...
        "algorithm": {"name": "基于库存阈值和消耗速度", "parameters": {"min_stock_threshold_source": "药品表min_stock_threshold字段", "daily_avg_calculation_period": "最近30天", "safety_stock_days": 7}},
...
def inventory_health_check() -> Dict[str, Any]:
    """库存系统健康检查（占位实现）"""
    return {
        "status": "placeholder", "module": "inventory",
