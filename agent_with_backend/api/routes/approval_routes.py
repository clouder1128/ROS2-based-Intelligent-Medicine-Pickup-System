                    ...
                    if not err:
                        ...
                        try:
                            ...
                            if cur.rowcount > 0:
                                ...
                                order_created = True
                                order_message = f"Order created, task ID: {task_id}, quantity: {quantity}"
                            else:
                                order_message = f"Insufficient stock, need {quantity}"
                        except Exception as db_error:
                            order_message = f"Database error: {str(db_error)}"
                            if conn:
                                conn.rollback()
                        finally:
                            if conn:
                        ...
                        order_message = f"Drug validation failed: {err}"
                else:
                    order_message = f"Drug not found: {drug_name}"
            else:
                order_message = "Approval has no drug specified"
        except Exception as e:
            order_message = f"Order creation error: {str(e)}"
        response_data = {"success": True, "message": "Approval approved successfully", "approval_id": approval_id, "doctor_id": data["doctor_id"], "approved_at": datetime.now().isoformat(), "order_created": order_created, "order_message": order_message}
        if notes:
