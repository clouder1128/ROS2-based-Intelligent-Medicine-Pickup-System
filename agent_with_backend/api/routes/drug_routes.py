...
def list_drugs():
    ...
    if name_filter is not None:
        if len(name_filter) > 100:
            return (
                jsonify({"success": False, "ok": False, "error": "Name filter too long (max 100 characters)", "code": "INVALID_INPUT"}),
                400,
            )
        import re
        if re.search(r'[;\\\'"`]', name_filter):
            return (
                jsonify({"success": False, "ok": False, "error": "Invalid characters in name filter", "code": "INVALID_INPUT"}),
                400,
            )
    ...
    try:
        ...
        return (jsonify({"success": False, "ok": False, "error": "Database error while fetching drugs", "code": "DB_ERROR"}), 500)
    finally:
        if conn:
            conn.close()
...
def get_drug(drug_id):
    ...
    try:
        ...
        row = cur.fetchone()
        if row is None:
            return (jsonify({"success": False, "ok": False, "error": f"Drug not found: id={drug_id}", "code": "DRUG_NOT_FOUND"}), 404)
        drug = dict(row)
        return jsonify({"success": True, "drug": drug})
    except Exception as e:
        print(f"Database error in get_drug: {e}")
        return (jsonify({"success": False, "ok": False, "error": "Database error while fetching drug", "code": "DB_ERROR"}), 500)
    finally:
        if conn:
            conn.close()
