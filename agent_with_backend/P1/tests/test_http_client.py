...
class TestPharmacyHTTPClient:
    ...
    async def test_get_drugs_async_success(self, client, mock_drugs_response):
        """测试异步获取药物列表成功"""
        with patch.object(
            client, "_make_request", AsyncMock(return_value=mock_drugs_response)
        ):
    ...
    def test_get_drugs_sync(self, client, mock_drugs_response):
        ...
        with patch.object(
            client,
            "get_drugs_async",
            AsyncMock(return_value=mock_drugs_response["drugs"]),
        ):
    ...
    async def test_get_drug_by_id_async_success(self, client, mock_drug_response):
        """测试异步根据ID获取药物成功"""
        with patch.object(
            client, "_make_request", AsyncMock(return_value=mock_drug_response)
        ):
    ...
    def test_get_drug_by_id_sync(self, client, mock_drug_response):
        ...
        with patch.object(
            client,
            "get_drug_by_id_async",
            AsyncMock(return_value=mock_drug_response["drug"]),
        ):
    ...
    async def test_create_approval_async_success(self, client, mock_approval_response):
        """测试异步创建审批成功"""
        with patch.object(
            client, "_make_request", AsyncMock(return_value=mock_approval_response)
        ):
            result = await client.create_approval_async(
                patient_name="张三", advice="建议使用布洛芬"
            )
            assert result == "AP-123456"

    @pytest.mark.asyncio
    async def test_create_approval_async_with_kwargs(
        self, client, mock_approval_response
    ):
        """测试异步创建审批带额外参数"""
        with patch.object(
            client, "_make_request", AsyncMock(return_value=mock_approval_response)
    ...
    def test_create_approval_sync(self, client, mock_approval_response):
        """测试同步创建审批"""
        with patch.object(
            client, "create_approval_async", AsyncMock(return_value="AP-123456")
        ):
            result = client.create_approval("张三", "建议使用布洛芬")
            assert result == "AP-123456"

    @pytest.mark.asyncio
    async def test_get_approval_async_success(
        self, client, mock_approval_detail_response
    ):
        """测试异步获取审批成功"""
        with patch.object(
            client,
            "_make_request",
            AsyncMock(return_value=mock_approval_detail_response),
        ):
    ...
    @pytest.mark.asyncio
    async def test_get_pending_approvals_async_success(
        self, client, mock_pending_approvals_response
    ):
        """测试异步获取待处理审批成功"""
        with patch.object(
            client,
            "_make_request",
            AsyncMock(return_value=mock_pending_approvals_response),
        ):
    ...
    @pytest.mark.asyncio
    async def test_get_pending_approvals_async_with_limit(
        self, client, mock_pending_approvals_response
    ):
        """测试异步获取待处理审批带限制"""
        with patch.object(
    ...
    def test_get_pending_approvals_sync(self, client, mock_pending_approvals_response):
        ...
        with patch.object(
            client,
            "get_pending_approvals_async",
            AsyncMock(return_value=mock_pending_approvals_response["approvals"]),
        ):
    ...
    async def test_approve_approval_async_success(self, client, mock_response_success):
        ...
        with patch.object(
            client, "_make_request", AsyncMock(return_value=mock_response_success)
        ):
    ...
    async def test_reject_approval_async_success(self, client, mock_response_success):
        ...
        with patch.object(
            ...
            result = await client.reject_approval_async(
                "AP-123456", "DR-001", "药物过敏"
            )
            assert result["success"] is True
    ...
    async def test_create_order_async_success(self, client, mock_order_response):
        ...
        with patch.object(
            client, "_make_request", AsyncMock(return_value=mock_order_response)
        ):
    ...
    def test_create_order_sync(self, client, mock_order_response):
        ...
        items = [{"id": 1, "num": 2}]
        with patch.object(
            client, "create_order_async", AsyncMock(return_value=mock_order_response)
        ):
    ...
    async def test_health_check_async_success(self, client, mock_health_response):
        """测试异步健康检查成功"""
        with patch.object(
            client, "_make_request", AsyncMock(return_value=mock_health_response)
        ):
    ...
    def test_health_check_sync(self, client, mock_health_response):
        """测试同步健康检查"""
        with patch.object(
            client, "health_check_async", AsyncMock(return_value=mock_health_response)
        ):
    ...
    async def test_make_request_network_error(self, client):
        """测试网络错误处理"""
        with patch(
            "httpx.AsyncClient", side_effect=httpx.ConnectError("Connection failed")
        ):
            result = await client._make_request("GET", "/api/drugs")
            assert result is None
    ...
    async def test_make_request_timeout(self, client):
        """测试超时错误处理"""
        with patch(
            "httpx.AsyncClient", side_effect=httpx.TimeoutException("Request timed out")
        ):
            result = await client._make_request("GET", "/api/drugs")
            assert result is None
    ...
    def test_invalid_json_response(self, client):
        ...
        with patch.object(
            client, "get_drugs_async", AsyncMock(return_value={"success": False})
        ):
            result = client.get_drugs()
            assert result == []
