...
class SymptomExtractor:
    ...
    def extract_patient_info(self, text: str) -> PatientInfo:
        ...
        # 提取过敏史
        allergies = []
        allergy_match = re.search(r'对([^过敏]+)过敏|过敏[于到]?\s*([^,，。！]+)', text)
        if allergy_match:
            allergen = allergy_match.group(1) or allergy_match.group(2)
            if allergen:
                # 分割多个过敏原（用逗号、顿号、空格等分隔）
                allergens = re.split(r'[、,，\s]+', allergen.strip())
                allergies.extend([a.strip() for a in allergens if a.strip()])

        # 支持列表式过敏
        multiple_allergies = re.findall(r'过敏[于到]?\s*([^,，。！]+)', text)
        for allergen in multiple_allergies:
            allergens = re.split(r'[、,，\s]+', allergen.strip())
            allergies.extend([a.strip() for a in allergens if a.strip()])
